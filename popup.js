...
async function scanTab() {
    ...
    // SO Set treated them as three different colors and rendered 3 separate color boxes for the exactsame hex value
    // To fix this I decoded to convert to hex first then count, should have done that previously, stupid me
    const colorInstance = {};
    retColorRaw.forEach(el => {
        [el.color, el.backgroundColor, el.borderColor].forEach(c => {
            const hex = rgbStringToHex(c);
            colorInstance[hex] = (colorInstance[hex] || 0) + 1;
        });
    });

    const allUniColors = Object.keys(colorInstance);
    ...
    document.querySelector("#all-colors").innerHTML = allUniColors.map(el =>
        `
        <li>
            <div class="color-box" style="--bg-color:${el};">
            <div class="color-box-text">
                <h4 style="color:${getContrastColor(el)}">${el}</h4>
                <p class="footnote" style="color:${getContrastColor(el)}">${colorInstance[el] || 0} instances</p>
            </div>
            <div class="clipboard-wrapper">
                <span class="copy-note">Copied</span>
                <button class="copy-to-clipboard" style="color:${getContrastColor(el)};">
    ...
    document.querySelector("#color-summary").innerHTML = allUniColors.map(el =>
        `
        <li>
            <div class="summary-color-box" style="--bg-color:${el}"></div>  
        </li>
        `
    ).join("");
